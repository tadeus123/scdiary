...
const COOKIE_USER = 'airsupdev_user';
const COOKIE_STATE = 'airsupdev_oauth_state';
const COOKIE_PATH = '/airsupdev';
...
function callbackUrl(req) {
  return `${getPublicOrigin(req)}/airsupdev/auth/google/callback`;
}
...
async function exchangeCode(req, code) {
  ...
  return {
    ...
    email: String(userJson.email).toLowerCase(),
...
function safeNext(next) {
  ...
  if (!path.startsWith('/airsupdev')) return '/airsupdev/oauth/done';
  if (path.startsWith('//')) return '/airsupdev/oauth/done';
  return path || '/airsupdev/oauth/done';
}
