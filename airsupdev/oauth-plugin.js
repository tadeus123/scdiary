...
function pluginAuthorizeUrl(req) {
  return `${publicOrigin(req)}/airsupdev/oauth/authorize`;
}

function authorizationServerMetadata(req) {
  const issuer = `${publicOrigin(req)}/airsupdev/oauth`;
  return {
    issuer,
    authorization_endpoint: pluginAuthorizeUrl(req),
    token_endpoint: `${publicOrigin(req)}/airsupdev/oauth/token`,
    registration_endpoint: `${publicOrigin(req)}/airsupdev/oauth/register`,
...
function protectedResourceMetadata(req) {
  return {
    resource: MCP_URL,
    authorization_servers: [`${publicOrigin(req)}/airsupdev/oauth`],
...
async function finishAuthorize(req, res, store, pending, googleUser) {
  if (!auth.isEmailAllowed(googleUser.email)) {
    return res.status(403).send(
      'This Airsupdev MCP is private. Your Google account is not on the allowlist. Ask the owner to add your email.'
    );
  }
...
async function handleAuthorize(req, res, store) {
  ...
  if (websiteUser && websiteUser.googleId) {
    if (!auth.isEmailAllowed(websiteUser.email)) {
  ...
  const next = `/airsupdev/oauth/authorize?${params.toString()}`;
  return res.redirect(`/airsupdev/connect?next=${encodeURIComponent(next)}`);
}
...
async function handleToken(req, res, store) {
  ...
  if (grant === 'authorization_code') {
    ...
    if (!user || !auth.isEmailAllowed(user.email)) {
      return res.status(403).json({ error: 'access_denied', error_description: 'email not allowlisted' });
    }
