...
import { createAgentGateway } from "@web-native-agent/server";
import express from "express";
import { createAirCartAgent } from "./agent.js";
import { getStatus, setStatus } from "./status.js";
export function buildAirCartCard(publicOrigin) {
    const origin = publicOrigin.replace(/\/$/, "");
    return {
        name: "AirCart Agent",
        description: "AirCart assistant for tademehl.com. Explains what AirCart is doing and how to connect a company agent to the website.",
        supportedInterfaces: [
            {
                url: `${origin}/a2a/v1`,
        ...
        skills: [
            {
                ...
                description: "Answers what AirCart is doing right now and how to install or connect an agent on the website.",
...
function renderAgentPage(origin, status) {
    return `<!doctype html>
<html lang="en">
<head>
  <meta charset="utf-8" />
...
export function createAirCartApp(options) {
    ...
    gateway.app.get("/agent", (_req, res) => {
        res.type("html").send(renderAgentPage(options.publicOrigin.replace(/\/$/, ""), getStatus()));
    });
    gateway.app.get("/agent/status.json", (_req, res) => {
        res.json(getStatus());
    });
    gateway.app.post("/agent/status", (req, res) => {
