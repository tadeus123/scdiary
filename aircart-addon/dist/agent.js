...
function wantsStatus(text) {
    return /what.*(doing|working|status|up to)|right now|currently|aircart|who are you|what is this/i.test(text);
}
    ...
    return async (input) => {
        ...
        if (!text) {
            return {
                ...
                taskId,
                contextId,
                message: "Ask me what AirCart is doing right now, what the product is, or how to connect an agent to tademehl.com.",
            };
        }
        ...
        if (/how (do i|to) (install|connect|add|setup|set up)/i.test(text)) {
            return {
                ...
                taskId,
                contextId,
                message: "To put AirCart on your website: 1) run the aircart-agent (or gateway) on a host, 2) reverse-proxy /.well-known/agent-card.json, /a2a/v1, and /agent to that host on tademehl.com, 3) add the HTML discovery link from USER_GUIDE.md, 4) point your company agent via function adapter or signed webhook. Full steps are in USER_GUIDE.md.",
            };
        }
