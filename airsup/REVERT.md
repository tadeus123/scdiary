...
Then run `airsup/sql/drop.sql` in the Supabase SQL editor (drops only `airsup_*` tables, including `airsup_china_*` if that slice was installed). To remove only the company onboarding, see `airsup/china/REVERT.md`.
