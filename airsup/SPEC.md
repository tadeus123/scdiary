...
The wait is the protocol. Hosting must hold `send_message` until `replied` or `ended` (or a true `failed`). Do not use Vercel request time limits as the conversation loop. Run waiters on a long-lived process so a normal Anna delay does not return `failed`.
