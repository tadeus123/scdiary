...
(async () => {
  ...
  assert.ok(widgetContents(WIDGET_URI).contents[0].text.includes('status-card'));
  assert.ok(widgetContents(WIDGET_URI).contents[0].text.includes('WAIT_PHASES'));
  assert.ok(widgetContents(WIDGET_URI).contents[0].text.includes('Talking to'));
  ...
  assert.ok(widgetContents(WIDGET_URI).contents[0].text.includes('endConversation'));
  assert.ok(widgetContents(WIDGET_URI).contents[0].text.includes('class="corner"'));
  ...
  assert.ok(widgetContents(WIDGET_URI).contents[0].text.includes('Waiting for reply'));
  ...
  assert.ok(!widgetContents(WIDGET_URI).contents[0].text.includes('Your message'));
  assert.ok(!widgetContents(WIDGET_URI).contents[0].text.includes('id="composer"'));
  assert.ok(!widgetContents(WIDGET_URI).contents[0].text.includes('id="draft"'));
  assert.ok(!widgetContents(WIDGET_URI).contents[0].text.includes('id="transcript"'));
  assert.ok(!widgetContents(WIDGET_URI).contents[0].text.includes('class="bubble"'));
  assert.ok(!widgetContents(WIDGET_URI).contents[0].text.includes('liveMessages'));
  assert.ok(!widgetContents(WIDGET_URI).contents[0].text.includes('renderMessages'));
  assert.strictEqual(toolList().tools[1]._meta['openai/toolInvocation/invoking'], 'Sending');
  assert.ok(toolList().tools[1].description.includes('high-bandwidth'));
  assert.ok(toolList().tools[1].description.includes('status progress'));
