...
const { WIDGET_URI, shouldMountConversationWidget } = require('./widget');
...
(async () => {
  ...
  assert.strictEqual(toolList().tools[1]._meta['openai/widgetAccessible'], undefined);
  assert.strictEqual(toolList().tools[2]._meta['openai/widgetAccessible'], undefined);
  assert.strictEqual(toolList().tools[1]._meta['openai/toolInvocation/invoking'], 'Talking on Airsup');
  assert.strictEqual(shouldMountConversationWidget({ args: { person_id: 'p1' } }), false);
  ...
  assert.ok(WIDGET_URI.includes('airsup-conversation-v19.html'));
  ...
  assert.ok(!payload.content[0].text.includes('widget'));
  assert.ok(!payload.content[0].text.startsWith('{'));
  assert.strictEqual(payload._meta['openai/resultCanProduceWidget'], false);
  assert.strictEqual(payload._meta['openai/outputTemplate'], undefined);
  assert.ok(!payload._meta.ui);
  ...
  assert.strictEqual(openCard._meta['openai/outputTemplate'], undefined);
  assert.strictEqual(openCard._meta['openai/resultCanProduceWidget'], false);
